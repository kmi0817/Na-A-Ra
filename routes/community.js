...
router.get("/clinics", async (req, res) => {
    try {
        ...
        res.send({ category: "clinics", postings: postings, text: "Hello" });
    } catch (error) {
        console.log("*** " + error);
    }
});
...
router.get("/clinics/:id", async (req, res) => {
    try {
        ...
        console.log(posting);
        res.send({ category: "clinics", posting: posting });
    } catch (error) {
        console.log("*** " + error);
    }
});
...
router.get("/questions", async (req, res) => {
    try {
        ...
        res.send({ category: "questions", postings: postings, text:"Hello2" });
    } catch (error) {
        console.log("*** " + error);
    }
});
...
router.get("/questions/:id", async (req, res) => {
    try {
        const posting = await Communities.findById(req.params.id);
        console.log(posting);
        res.send({ category: "questions", posting: posting });
    } catch (error) {
        console.log("*** " + error);
    }
});
